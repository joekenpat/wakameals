...
const CartItem = ({ extra, setCartData, cartData }) => {
  ...
  const handleAdd = async () => {
    ...
    let selectedExtra = await cartData.meal_extras.filter(
      (extras) => extras.id === extra.id
    );
    ...
    if (!isAvailable) {
      ...
      const index = await cartData.meal_extras.findIndex(
        (extras) => extras.id === extra.id
      );
      ...
      <div className="row">
        <div className="col-5">
          ...
          <p
            ...
            className="ml-2"
          >
            <Naira>{extra.price}</Naira>/{extra.measurement_quantity}
            {extra.measurement_type}
          </p>
        </div>
        <div className="col-3">
          <p
        ...
        >
          ...
          <div className="btn btn-style">{defaultMeasurement - 1}</div>
