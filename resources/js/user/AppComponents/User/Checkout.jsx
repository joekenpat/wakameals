...
import { MdHome, MdPerson, MdPhoneAndroid } from "react-icons/md";
...
const Checkout = (props) => {
  ...
  const onSuccess = (reference) => {
    ...
    setOpen1(false);
    setLoading(true);
    Axios.get(`/api/order/verify_payment?trxref=${config.reference}`, {
      headers: {
        Authorization: `Bearer ${parsedToken}`,
        "Content-Type": "application/json",
        Accept: "application/json",
  ...
  const onClose = () => {
    Axios.get(`/api/order/verify_payment?trxref=${config.reference}`).then(
      () => {
  ...
  useEffect(() => {
    if (typeof props.location.query !== "undefined") {
      ...
      Axios.get("/api/avail_pickup/list").then((res) => {
        setPickupLocation({
          ...pickupLocation,
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="my-5 container">
          <div className="row">
            <div className="col-lg-8 shadow p-2">
              <div className="container">
                ...
                <div className="mt-2 row">
                  ...
                  <div
                    onClick={() => setSelected("door_delivery")}
                    style={{
                      border: "1px solid gray",
                      backgroundColor:
                        selected === "door_delivery" ? "#ffeee3" : "white",
                    }}
                    className="col-lg-5 cursor mr-lg-1"
                  >
                    <div className="p-1">
                      ...
                      <div className="mt-2">
                        <div style={{ fontSize: "13px" }}>
                          <MdPerson
                            style={{ color: "#ff7417", fontSize: "15px" }}
                          />
                  ...
                  <div
                    ...
                    style={{
                      border: "1px solid gray",
                      backgroundColor:
                        selected === "pickup" ? "#ffeee3" : "white",
                    }}
                    className="col-lg-5 cursor ml-lg-1 mt-3 mt-lg-0"
                  >
                    <div className="p-1">
                      <h6 style={{ color: "#ff7417" }}>
            ...
            <div className="col-lg-3 mt-3 mt-lg-0 shadow p-2">
              <div>
                <h5>Order Options</h5>
                <hr className="mt-3" />
                <div
                ...
                {checked && (
                  <div className="mt-3">
                    ...
                    {days.map((data, index) => (
                      ...
                      >
                        <Checkbox
                          checked={
                            data === 1
                              ? checkedMonday.status
                              : data === 2
                              ? checkedTuesday.status
                              : data === 3
                              ? checkedWednesday.status
                              : data === 4
                              ? checkedThursday.status
                              : data === 5
                              ? checkedFriday.status
                              : data === 6
                              ? checkedSaturday.status
                              : checkedSunday.status
                          }
                          onChange={(e) =>
                            data === 1
                              ? handleChangeMonday(e, date.addDays(1))
                              : data === 2
                              ? handleChangeTuesday(e, date.addDays(2))
                              : data === 3
                              ? handleChangeWednesday(e, date.addDays(3))
                              : data === 4
                              ? handleChangeThursday(e, date.addDays(4))
                              : data === 5
                              ? handleChangeFriday(e, date.addDays(5))
                              : data === 6
                              ? handleChangeSaturday(e, date.addDays(6))
                              : handleChangeSunday(e, date.addDays(7))
                          }
                          inputProps={{ "aria-label": "primary checkbox" }}
                        />
                    ...
                    <div className="mt-2">
                      <h6 style={{ color: "#ff7417" }}>
                        Choose delivery times (7:05 AM to 5:15 PM
                      </h6>
                      <div style={{ fontSize: "13px" }}>
                        First Time (Leave empty if not applicable)
                      </div>
                      <input
                        name="first_time"
                    ...
                    <div className="mt-2">
                      <div style={{ fontSize: "13px" }}>
                        Second Time (Leave empty if not applicable)
                      </div>
                      <input
                        name="second_time"
      ...
      >
          ...
          <DialogContentText id="alert-dialog-cart">
            <div>
              {pickupLocation.data.map((data, index) => (
                <div
                  onClick={() => handlePickup(data.code, data)}
                  style={{ border: "1px gray solid", fontSize: "14px" }}
                  className="p-2 hover-location cursor mt-2"
                >
                  {data.address}, {data.town.name}, {data.lga.name},{" "}
                  {data.state.name}
                </div>
              ))}
            </div>
        ...
        <DialogActions>
          <Button
            onClick={handleClose}
            style={{ color: "#ff7417", fontWeight: "550" }}
          >
            CLOSE
          </Button>
        </DialogActions>
