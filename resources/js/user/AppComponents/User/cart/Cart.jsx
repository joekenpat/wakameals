  ...
  useEffect(() => {
    ...
    if (parsedCartItem !== undefined) {
      if (typeof parsedToken === "string") {
        ...
        Axios.post("/api/cart/sync", { items: parsedCartItem })
          .then((res) => {
            setCart(res.data.cart.items);
            setItem(res.data.cart);
            setLoading(false);
            ...
            >
              <Link to="/">
                <button
                  ...
                  className="btn shadow px-3 py-2"
                >
                  CONTINUE SHOPPING
                </button>
              </Link>
              {cart.length > 0 && (
                <Link
                  to={{ pathname: "/checkout", query: { price: item.total } }}
                >
                  <button
                    style={{
                      color: "white",
                    ...
                    className="btn ml-4 shadow px-3 py-2"
                  >
                    GO TO CHECKOUT
                  </button>
                </Link>
